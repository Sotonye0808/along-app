// Mock Database - TypeScript version
// This file contains all mock data for development and can be easily swapped with a real database

export const mockUsers: User[] = [
    {
        id: "1",
        userName: "chidi_travels",
        firstName: "Chidi",
        lastName: "Okafor",
        email: "chidi@example.com",
        password: "password234",
        verified: true,
        avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Chidi",
        bio: "Lagos navigator | Public transport expert | Sharing the best routes around Nigeria 🇳🇬",
        followers: 1245,
        following: ["2", "3"],
        likes: ["1", "2"],
        bookmarks: ["2"],
        createdAt: "2024-01-15T10:30:00Z",
    },
    {
        id: "2",
        userName: "ada_explorer",
        firstName: "Ada",
        lastName: "Nwosu",
        email: "ada@example.com",
        password: "password234",
        verified: true,
        avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Ada",
        bio: "Port Harcourt babe | Food & travel | Making Lagos traffic bearable one route at a time 😊",
        followers: 2512,
        following: ["1"],
        likes: ["1"],
        bookmarks: ["1", "2"],
        createdAt: "2024-02-20T14:20:00Z",
    },
    {
        id: "3",
        userName: "emeka_routes",
        firstName: "Emeka",
        lastName: "Igwe",
        email: "emeka@example.com",
        password: "password234",
        verified: true,
        avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Emeka",
        bio: "Abuja resident | Budget traveler | Your guide to affordable trips across Nigeria",
        followers: 856,
        following: ["1", "2"],
        likes: ["2"],
        bookmarks: [],
        createdAt: "2024-03-10T09:15:00Z",
    },
    {
        id: "4",
        userName: "Eddie",
        firstName: "Edward",
        lastName: "Ijeruh",
        email: "ijeruh20@gmail.com",
        password: "Password345",
        avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Edward",
        bio: "",
        followers: 0,
        following: [],
        likes: [],
        bookmarks: [],
        createdAt: "2025-10-28T12:03:46.715Z",
        verified: true,
    },
];

export const mockPosts: Post[] = [
    {
        id: "1",
        userId: "1",
        title: "Best Route from Ikeja to Victoria Island - Beat the Traffic!",
        routes: [
            {
                id: "r1",
                text: "Start at Ikeja Bus Stop. Take a BRT bus heading towards CMS. The dedicated lane saves you from go-slow! Fare: ₦300",
                links: [
                    {
                        text: "BRT Lagos Route Map",
                        url: "https://www.primerolagos.com",
                    },
                ],
                order: 1,
                vehicles: ["bus"],
                fare: 300,
                status: "verified",
            },
            {
                id: "r2",
                text: "Stop at Obalende. From there, hop on a yellow danfo or keke to VI. Keke is faster but costs ₦200, danfo is ₦150.",
                links: [],
                order: 2,
                vehicles: ["bus", "keke"],
                fare: 200,
                status: "unverified",
            },
            {
                id: "r3",
                text: "Get off at Adeola Odeku. If you're going to Lekki Phase 1, take another keke for ₦150. Total time: 45 mins (off-peak)",
                links: [
                    {
                        text: "Google Maps - VI",
                        url: "https://maps.google.com",
                    },
                ],
                order: 3,
                vehicles: ["keke"],
                fare: 150,
                status: "verified",
            },
        ],
        images: [
            "https://images.unsplash.com/photo-1578662996442-48f60103fc96",
            "https://images.unsplash.com/photo-1544620347-c4fd4a3d5957",
        ],
        tags: ["lagos", "ikeja", "vi", "brt", "traffic-hack"],
        likes: 342,
        dislikes: 8,
        comments: 45,
        bookmarks: 128,
        createdAt: "2024-10-20T15:30:00Z",
        updatedAt: "2024-10-21T09:15:00Z",
    },
    {
        id: "2",
        userId: "2",
        title: "Weekend Trip: Lagos to Ibadan via Public Transport",
        routes: [
            {
                id: "r4",
                text: "Head to Ojota Motor Park early morning (before 7 AM to avoid crowd). Look for ABC Transport or GUO buses. Fare: ₦2,500 - ₦3,000",
                links: [
                    {
                        text: "ABC Transport Schedule",
                        url: "https://abctransport.com",
                    },
                ],
                order: 1,
                vehicles: ["bus"],
                fare: 2500,
                status: "verified",
            },
            {
                id: "r5",
                text: "Journey takes 2-3 hours depending on traffic. Bus has AC and makes one stop at Sagamu. Bring snacks and water!",
                links: [],
                order: 2,
                vehicles: ["bus"],
                fare: 0,
                status: "pending",
            },
            {
                id: "r6",
                text: "Arrive at Iwo Road Terminal in Ibadan. From there, take okada (₦200) or keke (₦150) to Dugbe Market or UI if visiting campus.",
                links: [
                    {
                        text: "Things to do in Ibadan",
                        url: "https://www.google.com/travel/things-to-do/see-all?dest_mid=%2Fm%2F0fnky",
                    },
                ],
                order: 3,
                vehicles: ["bike", "keke"],
                fare: 150,
                status: "verified",
            },
        ],
        images: [
            "https://images.unsplash.com/photo-1464037866556-6812c9d1c72e",
            "https://images.unsplash.com/photo-1570125909232-eb263c188f7e",
        ],
        tags: ["lagos", "ibadan", "interstate", "weekend-trip", "budget-travel"],
        likes: 189,
        dislikes: 4,
        comments: 32,
        bookmarks: 76,
        createdAt: "2024-10-22T10:15:00Z",
        updatedAt: "2024-10-22T10:15:00Z",
    },
    {
        id: "3",
        userId: "3",
        title: "Navigating Abuja: Kubwa to Wuse Market Route",
        routes: [
            {
                id: "r7",
                text: "Start from Kubwa Phase 4. Take a keke to Kubwa main junction (₦100). Plenty of kekes available from 6 AM.",
                links: [],
                order: 1,
                vehicles: ["keke"],
                fare: 100,
                status: "verified",
            },
            {
                id: "r8",
                text: "At the junction, board a taxi or bus going to Berger Roundabout. Fare: ₦300 (shared taxi) or ₦200 (bus)",
                links: [],
                order: 2,
                vehicles: ["taxi", "bus"],
                fare: 300,
                status: "verified",
            },
            {
                id: "r9",
                text: "From Berger, take another bus directly to Wuse Market. Fare: ₦150. Total journey: 40-50 minutes depending on traffic.",
                links: [
                    {
                        text: "Wuse Market Guide",
                        url: "https://www.google.com/maps/place/Wuse+Market",
                    },
                ],
                order: 3,
                vehicles: ["bus"],
                fare: 150,
                status: "rejected",
            },
        ],
        images: ["https://images.unsplash.com/photo-1555196301-e08c30eb40a2"],
        tags: ["abuja", "kubwa", "wuse", "market", "daily-commute"],
        likes: 94,
        dislikes: 2,
        comments: 18,
        bookmarks: 41,
        createdAt: "2024-10-23T08:45:00Z",
        updatedAt: "2024-10-23T08:45:00Z",
    },
];

export const mockComments: PostComment[] = [
    {
        id: "1",
        postId: "1",
        userId: "2",
        text: "This route saved my life! Used it yesterday and avoided 2 hours of traffic. Thanks! 🙏",
        createdAt: "2024-10-20T16:45:00Z",
        likes: 12,
        dislikes: 0,
    },
    {
        id: "2",
        postId: "1",
        userId: "3",
        text: "The keke from Obalende to VI can be ₦250 during rush hour o. But still faster than sitting in traffic!",
        createdAt: "2024-10-20T17:00:00Z",
        likes: 8,
        dislikes: 1,
    },
    {
        id: "3",
        postId: "2",
        userId: "1",
        text: "GUO is more comfortable than ABC in my experience. But both are good options.",
        createdAt: "2024-10-22T11:30:00Z",
        likes: 15,
        dislikes: 2,
    },
    {
        id: "4",
        postId: "2",
        userId: "3",
        text: "Pro tip: Book your return ticket immediately you arrive in Ibadan. They fill up fast on Sundays!",
        createdAt: "2024-10-22T14:20:00Z",
        likes: 23,
        dislikes: 0,
    },
    {
        id: "5",
        postId: "3",
        userId: "1",
        text: "Abuja traffic is nothing compared to Lagos 😂 But this route is solid!",
        createdAt: "2024-10-23T09:15:00Z",
        likes: 5,
        dislikes: 3,
    },
];

export const mockLikes: Like[] = [
    {
        id: "1",
        postId: "1",
        userId: "2",
        type: "like",
    },
    {
        id: "2",
        postId: "1",
        userId: "3",
        type: "like",
    },
    {
        id: "3",
        postId: "2",
        userId: "1",
        type: "like",
    },
    {
        id: "4",
        postId: "3",
        userId: "2",
        type: "like",
    },
];

export const mockBookmarks: Bookmark[] = [
    {
        id: "1",
        postId: "1",
        userId: "2",
        createdAt: "2024-10-20T16:30:00Z",
    },
    {
        id: "2",
        postId: "1",
        userId: "3",
        createdAt: "2024-10-20T18:00:00Z",
    },
    {
        id: "3",
        postId: "2",
        userId: "1",
        createdAt: "2024-10-22T11:00:00Z",
    },
    {
        id: "4",
        postId: "2",
        userId: "3",
        createdAt: "2024-10-22T15:30:00Z",
    },
];

export const mockNotifications: AppNotification[] = [
    {
        id: "1",
        userId: "1",
        type: "like",
        message: "ada_explorer liked your post",
        postId: "1",
        read: false,
        createdAt: "2024-10-20T16:30:00Z",
    },
    {
        id: "2",
        userId: "1",
        type: "comment",
        message: "emeka_routes commented on your post",
        postId: "1",
        read: false,
        createdAt: "2024-10-20T17:00:00Z",
    },
    {
        id: "3",
        userId: "2",
        type: "like",
        message: "chidi_travels liked your post",
        postId: "2",
        read: true,
        createdAt: "2024-10-22T11:30:00Z",
    },
    {
        id: "4",
        userId: "3",
        type: "comment",
        message: "chidi_travels commented on your post",
        postId: "3",
        read: false,
        createdAt: "2024-10-23T09:15:00Z",
    },
];
